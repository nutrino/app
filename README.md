...
4. 서버에 자료가 있으면 **서버 → 로컬 단방향 동기화**를 선택합니다. 최초 적용에서
   같은 항목은 그대로 두고 차이만 반영합니다. 로컬에만 있는 항목은 백업 후 삭제합니다.
   서버가 비어 있으면 **로컬 → 빈 서버 최초 업로드**로 로컬을 유지한 채 처음 저장합니다.
...
연결 실패 시 입력값을 지우지 않습니다. 브라우저의 일반 URL 자동완성 대신 확장에
저장한 주소를 복원합니다. 기존 연결이 있는 경우 그 주소와 ID를 저장값으로 이전합니다.
...
최초 적용 완료 후에는 기존의 양방향 자동 동기화로 동작합니다.
